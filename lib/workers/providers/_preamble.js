...
const ENV_PREAMBLE = [
  ...
  "- Repo do Nook (~/dev/nook) — pode ler/editar código do Hub Node e do sidecar Python",
...
const SUPERPOWER_PREAMBLE = [
  ...
  "  1. DIAGNOSTICAR — use `curl localhost:3001/diagnostics`, leia logs (`tail /tmp/nook-server.log`, `tail /tmp/nook-core.log`), inspecione o código relevante (nook_core/browser.py, server.js, lib/*).",
  "  2. HIPOTETIZAR — diga em uma frase qual é a causa-raiz provável.",
  "  3. CORRIGIR — edite o código, ajuste config, reinicie o serviço (sidecar: `pkill -f 'nook_core.server'`; keeper respawna).",
...
const DB_POLICY = [
  "BANCO DE DADOS — vale SOMENTE para projetos em ~/dev/projetos/*. (O repo do próprio Nook Studio, ~/dev/nook, USA better-sqlite3 e está CORRETO — NÃO mexa no SQLite dele.)",
  "",
  ...
  "  (use o diretório-raiz — onde está package.json/.nook-project.json. Confira o retorno: {\"ok\":true} = sucesso; {\"error\":...} = falhou, leia e corrija. Depois rode /status pra confirmar.)",
  "",
