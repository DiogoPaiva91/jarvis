...
const CHATLIKE = (k) => k === "chat" || k === "code" || (typeof k === "string" && k.startsWith("code:"));

function listChats() {
  return registry.list().map((w) => w.toJSON()).filter(w => CHATLIKE(w.kind));
}

function createChat({ title, assistantId, provider, model, systemPrompt, kind }) {
  const w = registry.create({
    ...
    kind: (typeof kind === "string" && (kind === "code" || kind.startsWith("code:")) ? kind : "chat"),
  });
  return w.toJSON();
}
...
function getChat(id) {
  ...
  if (!w || !CHATLIKE(w.kind)) return null;
  return w.toJSON();
}
...
function deleteChat(id) {
  const w = registry.get(id);
  if (!w || !CHATLIKE(w.kind)) return false;
...
function migrateFromLocalStorage(payload) {
  ...
  const tx = db.transaction(() => {
    for (const [oldId, conv] of Object.entries(conversations)) {
      const asst = conv.assistantId === "nook"
